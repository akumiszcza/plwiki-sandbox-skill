...
- `obsidian-travel-skill` is the most relevant skill area for travel-note + Obsidian/BIGAI workflow. Keep the commit/push-without-asking rule visible there, not only in generic travel or booking notes.
- Default publishing rule for BIGAI/Obsidian/travel note work: push changes without asking after the edit is done, unless Adam explicitly asks to keep them local.
- Skill repos to keep locally cloned and easy to find: `/home/ubuntu/.openclaw/plwiki-sandbox-skill`, `/home/ubuntu/.openclaw/obsidian-travel-skill`, `/home/ubuntu/.openclaw/obsidian-concerts-skill`.
- Repo purposes for quick recall: `plwiki-sandbox-skill` = Polish Wikipedia/article workflow, `obsidian-travel-skill` = travel + Obsidian/BIGAI workflow, `obsidian-concerts-skill` = concert + Obsidian/BIGAI workflow.
- For all three skill repos, prefer pull/fetch first and default to commit + push without asking when making durable workflow changes Adam is meant to rely on.
