...
- For mature live articles, prefer very small, section-bounded passes (for example one wording gap, one refs-only pass, or one audit-fix pass), then validate, commit, and push before touching anything else.
...
- For already-live plwiki pages, default to incremental improvement of the published article, not a rewrite-from-scratch workflow. Preserve the existing sourced structure unless there is a strong reason to rebuild it.
- If Adam asks only to pull the live page back into the repo, do a clean sync/checkpoint first, without mixing in extra QA or cleanup. Name that commit plainly as `Sync <Title> from live plwiki`.
- For sync-only closeouts, prefer literal repo = live equality over trying to optimize away tiny diffs. Even a newline-at-EOF-only change is worth checkpointing when the goal is an exact raw sync back from plwiki.
...
- If the main article repo is blocked by unrelated dirt and cannot be refreshed safely, do not treat that dirty clone as authoritative. Continue in a fresh helper clone synced from remote, and keep the dirty main clone out of the editing path until it is cleaned up.
...
4. If Adam indicates the page is already live on plwiki, fetch the current live raw into the local `PL.mediawiki` before starting the next cleanup/fix pass.
...
- On active-science / cosmology articles, prefer durable explanatory value over hype. Very recent survey/program results should usually be phrased conservatively, with clear note when the signal depends on combining datasets or remains below standard discovery thresholds.
...
- If explicit access status must be shown (`otwarty`, `zamknięty`, `częściowy`), prefer converting the relevant ref to universal `{{Cytuj}}` and using `|dostęp=...` there after preview verification; do not force `|dostęp=` into `{{Cytuj pismo}}` or `{{Cytuj książkę}}` unless the exact variant is proven to support it.
- When converting a ref to universal `{{Cytuj}}`, remember that page ranges use `|s=` rather than `|strony=`; on plwiki `strony=` in `{{Cytuj}}` can trigger `Szablon_cytuj_do_sprawdzenia` even when the same range looked fine in another citation-template variant.
- Treat standalone `{{Otwarty dostęp}}` / `{{Paywall}}` templates as fallback only, not the default path, when a citation can be normalized to `{{Cytuj}}` without losing important bibliographic structure.
